...
if(isset($_POST['otp'])){
   if($_POST['otp']==$_SESSION['otp'] && time()-$_SESSION["time_otp"]<60){
            ...
            $p=mysqli_query($conn,"Insert into patient(Name,email,phone,Date,Password) values('".$row['Name']."','".$row['email']."',".$row['phone'].",'".date('yy-m-d h:i:s')."','".$row["password"]."');");
            //mysqli_commit($conn);
    ...
    echo "window.location.href='send_otp_patient.php';</script>";
}
else{
   $message="Invalid OTP";
...
<form action="otp_authenticate_patient.php" method="POST" class="form" autocomplete="off">
